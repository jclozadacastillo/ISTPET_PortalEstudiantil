...
function crearPasswordPreview() {
    document.querySelectorAll("[type='password']").forEach(item => {
        item.addEventListener("keyup", () => {
            ...
            if (!element) {
                if (item.parentElement && !item.parentElement.classList.contains("pvw-wrapper")) {
                    const wrapper = document.createElement("div");
                    wrapper.className = "position-relative d-block w-100 pvw-wrapper";
                    item.parentNode.insertBefore(wrapper, item);
                    wrapper.appendChild(item);
                }
                const iconColor = document.body.classList.contains("login-split-body") ? "text-white" : "text-primary";
                const elementHTML = `<a tabindex="-1" id="_pvw_${item.id}" href='javascript:;' onclick='__handlePasswordView("${item.id}")' class="pvw-password-toggle"><i class='bi-eye-fill ${iconColor}'></i></a>`;
